document.addEventListener('DOMContentLoaded', () => {
    ...
    function ehHoje(r) {
        ...
        const data = parsearData(r.data); data.setHours(0, 0, 0, 0);
    ...
    if (elData) elData.textContent = capitalize(new Date().toLocaleDateString('pt-BR', {
    ...
    const salasOcupadas = new Set(
        reservas.filter(r =>
            ...
            r.data && parsearData(r.data).setHours(0,0,0,0) === hoje.getTime()
        ).map(r => r.espaco)
    );
    ...
    // Todas as reservas confirmadas do usuário logado (não só hoje)
    const minhasHoje = reservas.filter(r =>
        String(r.usuarioId) === String(usuario.id) && r.status === 'Confirmada'
    ).sort((a, b) => {
    ...
    setEl('stat-reservas-sub',  proxima ? `próxima em ${proxima.data} às ${proxima.inicio}` : 'nenhuma confirmada');
    setEl('stat-membros',       totalMembros);
 
    // ── 3. Cards de reservas de hoje ──────────────────────────
    ...
    wrap.innerHTML = minhasHoje
        ...
        .map(r => {
            ...
            const qtdConv = (!ehMesa && r.convidados && r.convidados !== '-')
                ...
                : 0;
            const sub = ehMesa
                ? `${r.horario} · estação individual`
                : qtdConv > 0
                    ? `${r.horario} · ${qtdConv} convidado(s)`
                    : r.horario;
 
            return `
            <div class="reservation-card">
                <div>
                    <img src="assets/images/${r.espaco}.png" alt="${r.espaco}"
