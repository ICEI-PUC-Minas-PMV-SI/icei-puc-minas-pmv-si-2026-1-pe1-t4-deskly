...
function carregarUsuarios(filtro = '') {
    ...
    visiveis.forEach(usuario => {
        ...
        const badgePendente = !usuario.senhaDefinida
            ? `<span style="display:inline-block;margin-left:6px;font-size:0.7rem;background:#FEF3C7;color:#92400E;border-radius:4px;padding:1px 6px;font-weight:600;vertical-align:middle;">Pendente</span>`
            : '';
        ...
        tr.innerHTML = `
            <td>
                <div style="display:flex;align-items:center;gap:10px;">
                    <div id="${avatarId}" style="width:34px;height:34px;border-radius:50%;background:var(--color-primary,#2563EB);color:#fff;display:flex;align-items:center;justify-content:center;font-size:0.8rem;font-weight:700;flex-shrink:0;overflow:hidden;">
            ...
            <td>
                <div style="display:flex;align-items:center;gap:8px;">
                    <button type="button" class="btn-ver-detalhes btn-detalhes-usuario" title="Ver Detalhes" data-id="${usuario.id ?? ''}">
        ...
        carregarFotoDB(`foto_${usuario.id ?? usuario.email ?? 'anonimo'}`)
            .then(blob => {
                const avatarEl = document.getElementById(avatarId);
                if (!avatarEl) return;
                if (blob) {
                    avatarEl.innerHTML = `<img src="${URL.createObjectURL(blob)}" style="width:100%;height:100%;object-fit:cover;">`;
    ...
    tbody.querySelectorAll('.btn-ver-detalhes').forEach(btn =>
        btn.addEventListener('click', e => { e.preventDefault(); abrirModalDetalhes(btn); })
    );
...
function inicializarConvite() {
    ...
    btnConfirmar.addEventListener('click', async () => {
        ...
        if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            marcarErro(emailInput, true); valido = false;
        } else {
            marcarErro(emailInput, false);
        }
        if (!['Admin', 'Usuário'].includes(perfil)) {
            marcarErro(perfilInput, true); valido = false;
        } else {
            marcarErro(perfilInput, false);
        }
        if (!valido) { mostrarToast('Campos inválidos', 'Preencha todos os campos corretamente.', 'erro'); return; }

        const usuarios = obterUsuarios();
        if (usuarios.find(u => u.email === email)) {
            mostrarToast('E-mail já cadastrado', 'Este e-mail já está cadastrado no sistema.', 'erro');
            return;
        }
...
async function enviarConvitePorEmail(email, reenvio = false) {
    ...
    try {
        mostrarToast('Enviando convite', 'Aguarde enquanto o e-mail é enviado...', 'info');
        await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, { para_email: email, link_acesso: link, perfil_label: perfilLabel });
        mostrarToast(reenvio ? 'Convite reenviado' : 'Convite enviado', `E-mail ${reenvio ? 'reenviado' : 'enviado'} para ${email}.`, 'sucesso');
    } catch (erro) {
        console.error('Erro ao enviar email via EmailJS:', erro);
        mostrarToast('Erro ao enviar', 'Verifique o console para detalhes.', 'erro');
    }
}
...
function abrirModalDetalhes(btn) {
        ...
        if (src) {
            ...
            avatarPlaceholder.innerHTML = `
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="40" height="40" style="color:#6B7280;">
    ...
    carregarFotoDB(`foto_${id || usuario.email || 'anonimo'}`)
        .then(blob => renderizarAvatar(blob ? URL.createObjectURL(blob) : usuario.foto || ''))
...
function configurarModalDetalhes() {
    document.addEventListener('click', e => {
        if (e.target.matches('[data-modal="modal-detalhes-usuario"]') ||
            e.target.closest('[data-modal="modal-detalhes-usuario"]')) {
            document.getElementById('modal-detalhes-usuario')?.close();
        }
    });
}
...
function abrirModalRemover(email, nome) {
    ...
    btnNovo.addEventListener('click', () => {
        salvarUsuarios(obterUsuarios().filter(u => u.email !== email));
        modal.close();
        carregarUsuarios();
        mostrarToast('Usuário removido', 'A operação foi concluída com sucesso.', 'sucesso');
    });
}
...
function marcarErro(el, estado) {
    ...
    if (estado) {
        const limpar = () => { marcarErro(el, false); el.removeEventListener('input', limpar); el.removeEventListener('change', limpar); };
